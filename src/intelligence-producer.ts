...
export type IntelligenceJudgment={
  ...
  event_shock_transmission:'BULLISH'|'BEARISH'|'TWO_SIDED';
...
export function validateIntelligenceJudgment(raw:unknown,allowedSourceRefs:Set<string>):{judgment:IntelligenceJudgment|null;errors:string[]}{
  ...
  if(!['BULLISH','BEARISH','TWO_SIDED'].includes(String(raw.event_shock_transmission)))errors.push('event_shock_transmission is invalid');
  ...
  if(isObject(raw.horizon_slots)){
    for(const horizon of HORIZONS){
      ...
      if(!isObject(probabilities)||!exactKeys(probabilities,['BULL','RANGE','BEAR']))errors.push(`horizon_slots.${horizon}.probabilities must contain exactly BULL, RANGE and BEAR`);
      else{
        const bull=Number(probabilities.BULL),range=Number(probabilities.RANGE),bear=Number(probabilities.BEAR),values=[bull,range,bear];
        if(values.some(value=>!Number.isFinite(value)||value<0||value>100))errors.push(`horizon_slots.${horizon} scenario probabilities must be 0..100`);
        else{
          if(Math.abs(bull+range+bear-100)>0.02)errors.push(`horizon_slots.${horizon} scenario probabilities must total 100`);
          const selected=direction==='BULLISH'?bull:direction==='BEARISH'?bear:range;
          if(Math.abs(selected-Math.max(...values))>0.02)errors.push(`horizon_slots.${horizon}.direction must match the highest scenario probability`);
        }
      }
      ...
      if(typeof slot.basis!=='string'||!slot.basis.trim())errors.push(`horizon_slots.${horizon}.basis is mandatory`);
    }
  }
...
export async function produceIntelligence(ai:AiBinding,packet:unknown,allowedSourceRefs:Set<string>):Promise<{judgment:IntelligenceJudgment|null;normalized:JsonRecord|null;errors:string[];model:string}>{
  ...
  try{
    ...
    if(!validation.judgment&&validation.errors.length>0&&validation.errors.every(error=>error.startsWith('horizon_slots.'))&&isObject(parsed)){
      const repairPrompt='Your prior JSON was rejected because the D+1..D+5 forecast path was incomplete or invalid. Preserve all other evidence interpretation, raw scores, source_refs, regime, event shock, execution inputs and limitations. Using ONLY the supplied evidence packet, populate EACH horizon slot with exactly an evidence-grounded direction (BULLISH|RANGE|BEARISH), probabilities {BULL,RANGE,BEAR} each 0..100 totaling 100 within 0.02 percentage points, positive numeric zone_low, positive numeric zone_high with zone_high >= zone_low, and a brief basis. The selected direction MUST equal the highest scenario probability. NO TRADE or weak execution is not a reason to leave forecast-path slots empty. If the supplied PRICE_TECHNICALS evidence truly cannot support a path, set verification=UNAVAILABLE and data_adequate=false. Evidence packet:\n'+JSON.stringify(packet)+'\nPrior JSON:\n'+JSON.stringify(parsed);
      const repaired=await runGovernedInference(ai,{messages:[{role:'system',content:system},{role:'user',content:repairPrompt}],max_tokens:3200,temperature:0,chat_template_kwargs:{enable_thinking:false}});
      model=repaired.model;
      parsed=parseJson(repaired.raw);validation=validateIntelligenceJudgment(parsed,allowedSourceRefs);
    }
