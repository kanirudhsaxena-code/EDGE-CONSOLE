...
export type AcquisitionResultSync={
  ok:boolean;
  status:EngineResultSyncStatus;
  repository:string;
  workflow:string;
...
export async function sync5drAcquisitionResult(
  ...
  try{
    ...
    if(!runsResponse.ok)return {ok:false,status:'UNAVAILABLE',repository,workflow,detail:`GitHub acquisition workflow lookup returned HTTP ${runsResponse.status}`};
    ...
    if(String(run.conclusion)!=='success')return {ok:false,status:'FAILED',repository,workflow,workflow_run_id:runId,detail:'5DR acquisition workflow completed unsuccessfully before governed handoff'};
    ...
    if(!jobsResponse.ok)return {ok:false,status:'UNAVAILABLE',repository,workflow,workflow_run_id:runId,detail:`GitHub acquisition job lookup returned HTTP ${jobsResponse.status}`};
    const jobsBody=await jobsResponse.json();
    const jobs=isObject(jobsBody)&&Array.isArray(jobsBody.jobs)?jobsBody.jobs.filter(isObject):[];
    const job=jobs.find(item=>String(item.name)==='acquire')??jobs[0];
    if(!job||!Number(job.id))return {ok:false,status:'UNAVAILABLE',repository,workflow,workflow_run_id:runId,detail:'5DR acquisition workflow job was not found'};
    ...
    if(!logsResponse.ok)return {ok:false,status:'UNAVAILABLE',repository,workflow,workflow_run_id:runId,detail:`GitHub acquisition job logs returned HTTP ${logsResponse.status}`};
    const logs=await logsResponse.text();
    const matches=[...logs.matchAll(/EDGE_CONSOLE_MARKET_EVIDENCE_B64::([A-Za-z0-9+/=]+)/g)];
    const marker=matches.at(-1)?.[1];
    if(!marker)return {ok:false,status:'UNAVAILABLE',repository,workflow,workflow_run_id:runId,detail:'Validated 5DR acquisition handoff marker was not found in workflow logs'};
    let evidence:unknown;
    try{evidence=JSON.parse(decodeBase64Utf8(marker))}catch{return {ok:false,status:'UNAVAILABLE',repository,workflow,workflow_run_id:runId,detail:'Validated 5DR acquisition handoff could not be decoded'}};
    if(!isObject(evidence)||String(evidence.request_id)!==requestId)return {ok:false,status:'UNAVAILABLE',repository,workflow,workflow_run_id:runId,detail:'Validated 5DR acquisition handoff did not match the governed request'};
    return {ok:true,status:'SUCCEEDED',repository,workflow,workflow_run_id:runId,evidence};
  }catch{
    return {ok:false,status:'UNAVAILABLE',repository,workflow,detail:'GitHub Actions acquisition synchronization failed'};
  }
}
