...
import { initialAutonomousEvidenceState } from './autonomous-evidence';
...
async function createScreenshotReadyRequest(request: Request, env: Env): Promise<Response> {
  ...
  const metadata = {
    ...
    user_evidence_readiness: { status: 'SCREENSHOTS_READY', declared_categories: readiness.declared, required_categories: REQUIRED_USER_5DR_EVIDENCE_CATEGORIES, assessed_at: new Date().toISOString(), basis: 'USER_DECLARATION' },
  ...
  return json({ ok: true, request: { request_id: requestId, engine: '5DR', batch_id: batchId, provenance_mode: provenanceMode, framework_version: '5DR_V2_1', output_contract_version: '5DR_V2_1_2', status: 'READY_FOR_ENGINE', metadata }, next_step: 'Acquire and verify system-owned 5DR evidence' }, 201);
}
...
async function saveNormalizedEvidence(request: Request, env: Env, requestId: string): Promise<Response> {
  ...
  if (current.adapter_stage !== 'AUTONOMOUS_EVIDENCE_READY' && current.adapter_stage !== 'NORMALIZATION_BLOCKED') return json({ error: 'system-owned evidence is not ready for normalization', adapter_stage: current.adapter_stage ?? null, next_step: 'Acquire and verify autonomous evidence first' }, 409);
  ...
  return json({ ok: executable, request_id: requestId, normalized_items: evidence.length, adapter_stage: metadata.adapter_stage, blockers: assessment, next_step: executable ? 'Execute governed 5DR runner' : 'Supply missing or resolve conflicting normalized market inputs' }, executable ? 200 : 409);
}
...
async function executionPacket(env: Env, requestId: string): Promise<Response> {
  ...
  if (metadata.adapter_stage !== 'NORMALIZED_READY' || !Array.isArray(metadata.normalized_evidence) || !metadata.normalized_evidence.length) return json({ error: 'request is not normalization-ready', blockers: metadata.normalization_assessment ?? null, next_step: 'Complete autonomous evidence and normalized market evidence before execution' }, 409);
