...
function validate5drResult(result: unknown): string[] {
  ...
  if (result.assessment_snapshot_complete !== true) errors.push('result.assessment_snapshot_complete must be true');
  if (result.recommendation_ledger_complete !== true) errors.push('result.recommendation_ledger_complete must be true');
  ...
  else for (const horizon of requiredHorizons) if (!(horizon in horizons)) errors.push(`result.horizon_slots.${horizon} is mandatory`);
  return errors;
}
...
function validate5drEnvelope(body: unknown): string[] {
  ...
  if (!['SUCCESS', 'PARTIAL', 'FAILED', 'SHADOW'].includes(String(body.status))) errors.push('status must be SUCCESS, PARTIAL, FAILED or SHADOW');
  if (!isNonEmptyString(body.generated_at) || Number.isNaN(Date.parse(body.generated_at))) errors.push('generated_at must be a valid ISO timestamp');
  const provenance = body.provenance;
  if (!isObject(provenance)) errors.push('provenance is mandatory');
  else {
    if (!['MANUAL', 'HYBRID', 'AUTOMATED'].includes(String(provenance.mode))) errors.push('provenance.mode must be MANUAL, HYBRID or AUTOMATED');
...
async function uploadEvidence(request: Request, env: Env): Promise<Response> {
  ...
  let form: FormData;
  try { form = await request.formData(); } catch { return json({ error: 'Invalid multipart form data' }, 400); }
  ...
  if (!['MANUAL', 'HYBRID', 'AUTOMATED'].includes(provenanceMode)) return json({ error: 'provenance_mode must be MANUAL, HYBRID or AUTOMATED' }, 422);
  if (!files.length) return json({ error: 'At least one evidence file is required' }, 422);
  if (files.length > MAX_EVIDENCE_FILES) return json({ error: `Maximum ${MAX_EVIDENCE_FILES} evidence files per upload` }, 413);
  for (const file of files) {
    if (!ALLOWED_EVIDENCE_TYPES.has(file.type)) return json({ error: `Unsupported evidence type: ${file.type || 'unknown'}`, file: file.name }, 415);
    if (file.size <= 0 || file.size > MAX_EVIDENCE_FILE_BYTES) return json({ error: 'Each evidence file must be between 1 byte and 10 MB', file: file.name }, 413);
  }
  const capturedAt = capturedAtRaw && !Number.isNaN(Date.parse(capturedAtRaw)) ? new Date(capturedAtRaw).toISOString() : new Date().toISOString();
  const now = new Date();
  ...
  try {
    for (const file of files) {
      ...
      await env.EVIDENCE_BUCKET.put(objectKey, file.stream(), { httpMetadata: { contentType: file.type }, customMetadata: { engine, provenance_mode: provenanceMode, upload_id: uploadId, batch_id: uploadBatchId, original_filename: filename, captured_at: capturedAt } });
      storedKeys.push(objectKey);
      await sql`insert into evidence_uploads (upload_id, batch_id, engine, provenance_mode, object_key, file_name, mime_type, size_bytes, captured_at, status, metadata) values (${uploadId}, ${uploadBatchId}, ${engine}, ${provenanceMode}, ${objectKey}, ${filename}, ${file.type}, ${file.size}, ${capturedAt}, 'STAGED', ${JSON.stringify({ storage: 'R2', bucket_binding: 'EVIDENCE_BUCKET' })}::jsonb)`;
      uploaded.push({ upload_id: uploadId, file_name: filename, mime_type: file.type, size_bytes: file.size, captured_at: capturedAt, status: 'STAGED' });
    }
  } catch (error) {
...
async function api(request: Request, env: Env): Promise<Response> {
  ...
  if (url.pathname === '/api/runs/latest' && request.method === 'GET') {
    ...
    const sql = neon(env.DATABASE_URL); const engine = url.searchParams.get('engine');
    const runs = engine ? await sql`select run_id, engine, contract_version, framework_version, status, provenance_mode, freshness_at, generated_at, published, result, warnings from analysis_runs where published = true and engine = ${engine} order by generated_at desc limit 20` : await sql`select run_id, engine, contract_version, framework_version, status, provenance_mode, freshness_at, generated_at, published from analysis_runs where published = true order by generated_at desc limit 20`;
    return json({ runs });
  }
  if (url.pathname === '/api/5dr/latest' && request.method === 'GET') {
    ...
    const sql = neon(env.DATABASE_URL); const rows = await sql`select run_id, contract_version, framework_version, status, provenance_mode, sources, freshness_at, generated_at, result, warnings from analysis_runs where engine = '5DR' and published = true order by generated_at desc limit 1`;
...
export default { async fetch(request: Request, env: Env): Promise<Response> { const url = new URL(request.url); if (url.pathname.startsWith('/api/')) { try { return await api(request, env); } catch (error) { console.error(error); return json({ error: 'Internal server error' }, 500); } } return env.ASSETS.fetch(request); } };
